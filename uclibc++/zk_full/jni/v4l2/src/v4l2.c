                       ...
                       int pixelformat)
{
    ...
    //if (v4l2_ioct(s->fd, VIDIOC_S_FMT, &fmt))
...
{
    const struct video_data *s = (const struct video_data *)ctx->priv_data;
...
static int mmap_init(DeviceContex_t *ctx)
{
    ...
    s->buffers = req.count;
    s->buf_start = (void**)calloc(1, s->buffers * sizeof(void *));
    if (!s->buf_start) {
        DEMO_ERR("Cannot allocate buffer pointers\n");
        return -ENOMEM;
    }
    s->buf_len = (unsigned int*)calloc(1, s->buffers * sizeof(unsigned int));
    if (!s->buf_len) {
...
int v4l2_dev_init(DeviceContex_t **ctx, char *path)
{
    ...
    DeviceContex_t *c = (DeviceContex_t *)calloc(1, sizeof(DeviceContex_t));
    struct video_data *s = (struct video_data *)calloc(1, sizeof(struct video_data));
    ...
    *ctx = c;
    ...
    DEMOFile = fopen("/mnt/myH264.es", "w+");
    return 0;
}
...
void save_file(void *buf, uint length,char type)
{
    ...
    switch(type){
    case 0:
        snprintf(FileName, sizeof(FileName) - 1, "/mnt/myusb.img");
        DEMOFile = fopen(FileName, "w+");
       break;
    case 1:
        snprintf(FileName, sizeof(FileName) - 1, "/mnt/myusb.es");
