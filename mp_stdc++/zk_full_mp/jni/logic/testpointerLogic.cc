...
static void onProtocolDataUpdate(const SProtocolData &data) {
    mPointer1Ptr->setTargetAngle(data.pointer_angle);
}
...
static bool onUI_Timer(int id){
	...
	if(id == 0){
	    if (!mButtonswPtr->isSelected()) {
            float angle = rand() % 240;
            mPointer1Ptr->setTargetAngle(angle);
	    }
	}
	return true;
}
...
static bool onButtonClick_Button4(ZKButton *pButton) {
    EASYUICONTEXT->openActivity("helpPointerActivity");
    return false;
}

static bool onButtonClick_Buttonsw(ZKButton *pButton) {
    pButton->setSelected(!pButton->isSelected());
    mTextview1Ptr->setTextTr(!pButton->isSelected() ? "random": "serial_port");
    mTextview2Ptr->setVisible(pButton->isSelected());
    return false;
}
